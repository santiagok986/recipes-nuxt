<template>
  <v-app>
    ...
    <v-app-bar app :clipped-left="$vuetify.breakpoint.lgAndUp"  color="grey lighten-4" flat>
      ...
      <div v-if="$auth.loggedIn">
        ...
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn color="primary" @click="logout()" small>Cerrar Sesi&oacute;n</v-btn>
          </v-card-actions>
        </v-card>
        </v-menu>
      </div>
      ...
      <div v-else>
        <v-btn text color="primary" @click="dialog=true; type='app-forms-login'">Inciar Sesi&oacute;n</v-btn>
...
export default {
 /// middleware:"initData",
  ...
  methods:{
    ...
    // test(){
    //   this.$store.dispatch("snackbars/setSnack", {text:'sreste', color:'info'})
    // }
  }
}
</script>
